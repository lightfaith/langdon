#!/usr/bin/python3
"""
Decryption Oracle for Cryptopals 4.27 - CBC chosen ciphertext attack

The oracle decrypts given message, key == IV. 

Langdon uses the fact that if it can provide any ciphertext to
decryption routine, IV can be revealed.
"""
...
class OracleThread(Thread):
    ...
    def __init__(self, payloads, condition, break_on_success=False, peers=None, ** kwargs):
        ...
        self.params['iv'] = self.params['key']
        """"""
    ...
    def run(self):
        ...
        for payload_id, payload in self.payloads:
            ...
            key = self.params['key']
            iv = self.params['iv']
            aes = AES(mode='cbc', ciphertext=payload,
                      key=key, iv=iv, ignore_padding=True)
            aes.decrypt()
